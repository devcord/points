...
class Helper {
  ...
  public static validation = async (ctx: BaseContext, next: Function) => {
    if (ctx.invalid) {
      let body: { details: Array<object> } =
        ctx.invalid.body || ctx.invalid.query || ctx.invalid.params;
      let response: Array<object | undefined | null> =
        body && body.details ? body.details : [];
